...
def _gerar_registro_quina(pred, concurso_data):
    """Gera um registro de benchmark para Quina."""
    ...
    for _, r in historico_df.iterrows():
        for c in cols[:5]:
            try:
                ...
                continue
    # Fallback padrão da Quina 1..80
    return sorted(vals) if vals else list(range(1, 81))

def _predicao_baseline_random_quina(data_palpite, dominio):
    import random
    nums = sorted(random.sample(dominio, 5))
    return {"data": data_palpite, "modelo": "baseline_random", "jogo": nums, "simulada": True}

def _predicao_baseline_freq_quina(concurso_data, historico_df, data_palpite):
    import random
...
def _gerar_predicoes_simuladas(concurso_data, historico_df):
    ...
    dominio = _dominio_numeros(historico_df)
    return [
        _predicao_baseline_random_quina(data_palpite, dominio),
        _predicao_baseline_freq_quina(concurso_data, historico_df, data_palpite)
    ]
...
def benchmark():
    ...
    for _, row in df_real.iterrows():
        ...
        for pred in palpites_validos:
            registro = _gerar_registro_quina(pred, concurso_data)
...
def _calcular_faixas_acertos_quina(df):
    ...
    for modelo in df["modelo"].unique():
        ...
        faixas_acertos[modelo] = {
            "2_ou_mais": sum(dados_modelo >= 2),
            "3_ou_mais": sum(dados_modelo >= 3),
            "4_ou_mais": sum(dados_modelo >= 4),
            "5_acertos": sum(dados_modelo == 5)
        }
    return faixas_acertos

def _gerar_relatorio_markdown_quina(resumo, faixas_acertos, df_full):
    """Gera o relatório markdown com as estatísticas."""
    melhor_modelo = resumo.loc[resumo["media_acertos"].idxmax()]
    
    with open(SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write("# 🎯 Benchmark Summary - Quina\n\n")
        ...
        f.write("| Modelo | 2+ acertos | 3+ acertos | 4+ acertos | 5 acertos (Quina) |\n")
        f.write("|--------|------------|------------|------------|-------------------|\n")
        
        for modelo, faixas in faixas_acertos.items():
            f.write(f"| {modelo} | {faixas['2_ou_mais']} | {faixas['3_ou_mais']} | {faixas['4_ou_mais']} | {faixas['5_acertos']} |\n")
        f.write("\n## 🧪 Predições Simuladas\n\n")
...
def gerar_summary(df):
    ...
    melhor_modelo = _gerar_relatorio_markdown_quina(resumo, faixas_acertos, df)
