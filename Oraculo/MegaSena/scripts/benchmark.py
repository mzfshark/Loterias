...
def _gerar_grafico_interativo_megasena(resumo, df):
    ...
    # 4. Pie Chart - Faixas de Premiação (Quadra, Quina, Sena)
    faixas_labels = []
    faixas_values = []
    ...
    for modelo in resumo["modelo"]:
        ...
        if sena > 0:
            faixas_labels.append(f"{modelo} - Sena")
            faixas_values.append(sena)
    ...
    fig.update_layout(
        title={
            'text': "🎯 MegaSena - Análise Interativa de Performance dos Modelos",
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 16, 'color': MEGASENA_COLOR}
        },
        showlegend=True,
        height=800,
        font=dict(size=10),
    ...
    print(f"📈 Gráfico interativo MegaSena salvo em: {CHART_HTML}")
